"""Stratified outcome panels for the expanded Dragapult learner-root cohort."""
...
def selected_roots() -> list[str]:
    ...
    for row in P.read_jsonl(C.PUBLIC):
        if not P.discovery_episode(int(row["source"]["episode_id"])):
            continue
    ...
    if len(selected) < P.ROOTS:
        raise P.PanelError(f"only {len(selected)} stratified discovery roots")
    return selected
