"""Public-only relational features for the isolated Qu-v2A prototype.

This module deliberately lives outside :mod:`agent`.  Nothing in the shipped
dispatcher imports it, and it does not alter the feature contract of Qu-v1.
...
# contract, not loose implementation details.  Source/data hashes below catch
# logic or generated-database drift even when somebody forgets to bump one.
...
def compute_feature_dependency_hashes() -> tuple[tuple[str, str], ...]:
    """Return ordered SHA-256 hashes for every transitive feature input."""
    ...
    for relative in FEATURE_DEPENDENCY_PATHS:
        ...
        try:
            ...
            raise RuntimeError(
                f"Qu-v2A feature dependency is unavailable: {relative}"
            ) from exc
...
def feature_dependency_fingerprint(
    ...
    """Hash a path-bound dependency manifest with deterministic framing."""
    manifest = (compute_feature_dependency_hashes() if hashes is None
                else tuple(hashes))
    payload = b"ptcg.qu-v2a.feature-dependencies.v1\0" + b"\0".join(
...
def assert_feature_dependency_lock() -> str:
    """Fail if dependency files changed after these modules were imported."""
    current = compute_feature_dependency_hashes()
    if current != FEATURE_DEPENDENCY_HASHES:
        raise RuntimeError(
            "Qu-v2A feature dependencies changed after import; restart and "
