"""Paired field gate for bounded within-turn Crushing Hammer sequencing."""
...
RUN = ROOT / "tools/checkpoints/dragapult-hammer-sequence-v1-20260812/gameplay"
LOCK = RUN / "lock.json"
...
PATHS = {
    ...
    "behavior": ROOT / "tools/checkpoints/dragapult-hammer-sequence-v1-20260812/top-pilot-behavior.json",
}
...
class HammerController:
    ...
    def act(self, obs: dict) -> list[int]:
        ...
        try:
            ...
            if view.select_type == ST_MAIN:
                ...
                ordered = D._guard_hammer_sequencing(
                    view, logits, base,
                ) if self.enabled else base
                self.counts["hammer_sequence_guards"] += int(ordered != base)
                action = D._guard_phantom_completion(view, ordered)
                self.counts["completion_guards"] += int(action != ordered)
    ...
    def diagnostics(self):
        ...
        for key in ("fallbacks", "repairs", "hammer_sequence_guards", "completion_guards"):
...
def build_lock():
    if any(path.exists() for path in (LOCK, ATTEMPT, RESULT)):
        raise GateError("Hammer sequence gate already locked or consumed")
    missing = [str(path) for path in PATHS.values() if not path.is_file()]
    if missing:
        raise GateError(f"missing artifacts: {missing}")
    behavior = json.loads(PATHS["behavior"].read_text())
    if behavior["head"]["main_sha256"] != BASE.file_sha256(PATHS["main"]):
        raise GateError("behavior audit MAIN differs from gameplay MAIN")
    deck = load_deck(); qu = COMMON._load_net(PATHS["qu"], "Qu-v2B")
    opponents, _ = BASE.make_opponents(FIELD.current_field(), qu, "hammer-sequence-lock")
    schedule = build_paired_schedule(opponents, GAMES_PER_ARM, seed=SEED)
    payload = {
        "schema": "ptcg.dragapult-hammer-sequence.gameplay-lock.v1",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "written_before_engine_outcomes": True,
        "candidate": "elite MAIN/CARD plus completion and bounded Hammer ordering",
        "control": "exact dragapult-v2: elite MAIN/CARD plus completion",
        "intervention": (
            "when MAIN argmax is Crushing Hammer, choose highest-logit safe setup: "
            "ability, evolve, Pokemon play, Poffin, Night Stretcher, Poke Pad, "
            "or Jamming Tower; exclude attachments/supporters/attacks/retreat/END/"
            "Ultra Ball/Unfair Stamp; decode again after each action"
        ),
        "protocol": {
            ...
            "pass_authority": "fresh confirmation only; no direct package authority",
        },
...
def load_lock():
    ...
    if value.get("schema") != "ptcg.dragapult-hammer-sequence.gameplay-lock.v1" or claimed != canonical(value):
        raise GateError("Hammer sequence lock self-hash failed")
...
def run(quiet: bool):
    ...
    if ATTEMPT.exists() or RESULT.exists():
        raise GateError("Hammer sequence attempt already consumed")
    write_new(ATTEMPT, {
        "schema": "ptcg.dragapult-hammer-sequence.gameplay-attempt.v1",
        "created_at": datetime.now(timezone.utc).isoformat(),
    ...
    for arm in ("candidate", "control"):
        opponents, field_controller = BASE.make_opponents(FIELD.current_field(), qu, f"hammer-{arm}")
        schedule = build_paired_schedule(opponents, GAMES_PER_ARM, seed=SEED)
        if canonical(schedule_manifest(schedule, opponents)) != lock["schedule_manifest_sha256"]:
            raise GateError(f"schedule drifted: {arm}")
        controller = HammerController(main, card, qu, deck, f"hammer/{arm}", arm == "candidate")
        value = EVAL.run_series(
            f"hammer/{arm}", controller, deck, opponents, schedule,
    ...
    valid = all(row["valid"] for row in diagnostics.values()) and diagnostics["candidate"]["learner"]["hammer_sequence_guards"] > 0
    passed = bool(
        valid and overall["mean_delta"] > 0 and overall["ci95"][0] > -0.025
        and all(row["mean_delta"] >= -0.04 for row in slices.values())
    )
    payload = {
        "schema": "ptcg.dragapult-hammer-sequence.gameplay-result.v1",
