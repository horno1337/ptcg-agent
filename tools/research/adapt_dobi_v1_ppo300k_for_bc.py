"""Re-containerize the fixed PPO terminal state for the strict BC loader."""
...
SOURCE = (
    ROOT / "tools/checkpoints/dobi-v1-mirror-league-300k/training/"
...
def main() -> int:
    ...
    if (
        source.get("completed_updates") != 390
        or not isinstance(state, dict)
        or source.get("state_dict_sha256") != TRAIN._state_dict_sha256(state)
    ):
        raise SystemExit("PPO terminal checkpoint contract failed")
    payload = {
    ...
    if (
        ...
        or any(not torch.equal(state[name], verify["state_dict"][name]) for name in state)
    ):
        raise SystemExit("BC adapter changed the PPO terminal state")
    print(json.dumps({
        "output": str(OUTPUT),
