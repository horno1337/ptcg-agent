"""Stream one official daily archive and extract exact-c20 Grimmsnarl games.

The ZIP is never expanded wholesale.  Every replay is parsed once, registered
decks are recovered from the logged 60-card actions, and only games containing
the frozen Dobi/MD Grimmsnarl list are materialized.  The manifest retains the
teacher, outcome, opponent hash, and content hashes needed for a later
game-disjoint training lock; it grants no training or promotion authority.
"""
...
SCHEMA = "ptcg.grim.daily-archive-extraction.v1"
...
def extract(archive_path: Path, output: Path, skip_dirs: list[Path]) -> dict[str, Any]:
    ...
    teacher_outcomes: dict[str, Counter[str]] = {}
    opponents: Counter[str] = Counter()
    rows: list[dict[str, Any]] = []
    seen_ids: set[int] = set()
    target = md_v1.TARGET_DECK
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        for position, info in enumerate(members, 1):
            ...
            for seat in target_seats:
                ...
                teachers[teacher] += 1
                teacher_outcomes.setdefault(teacher, Counter())[outcome] += 1
            ...
            counters[
                "existing_episode_duplicates" if duplicate else "new_exact_games"
            ] += 1
            destination = None
            if not duplicate:
    ...
    payload = {
        ...
        "teacher_outcomes": {
            teacher: dict(counts) for teacher, counts in sorted(
                teacher_outcomes.items(), key=lambda item: (-sum(item[1].values()), item[0])
            )
        },
...
def main() -> int:
    ...
    print(json.dumps({
        ...
        "teacher_outcomes": result["teacher_outcomes"],
