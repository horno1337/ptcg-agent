"""Build the exact recent-data Dobi-v1 BC ladder candidate."""
...
WEIGHTS = ROOT / "tools/checkpoints/dobi-v1-bc-recent-v1/model/candidate-qu-v2a-weights.npz"
WEIGHTS_SHA256 = "aa07ffd146e39f3409341a8ed46ccc2047e7fc92827e244fb75a1b6f4b04aab2"
OLD_WEIGHTS_SHA256 = "bf93b3b7c114c0f406dea549ea8b7ac0aa783e0e80b3814fa4ea42ac412f679c"
MIRROR_RESULT = ROOT / "tools/checkpoints/dobi-v1-bc-recent-v1/mirror-vs-dobi/result.json"
OUTPUT = ROOT / "submission-dobi-v1-bc-recent-v1-unsigned.tar.gz"
MANIFEST = ROOT / "tools/checkpoints/dobi-v1-bc-recent-v1/package-manifest.json"
...
def build(output: Path, manifest: Path) -> dict:
    ...
    for path in (BASE, WEIGHTS, MIRROR_RESULT):
        if not path.is_file():
            raise BuildError(f"bound artifact missing: {path}")
    if sha256(BASE) != BASE_SHA256 or sha256(WEIGHTS) != WEIGHTS_SHA256:
        raise BuildError("frozen Dobi-v1 archive or BC weights drifted")
    mirror = json.loads(MIRROR_RESULT.read_text(encoding="utf-8"))
    if (
        mirror.get("decision", {}).get("valid") is not True
        or mirror.get("decision", {}).get("noninferior") is not True
    ):
        raise BuildError("required mirror preservation gate did not pass")
    output.parent.mkdir(parents=True, exist_ok=True)
    manifest.parent.mkdir(parents=True, exist_ok=True)
    partial = output.with_name(f".{output.name}.partial")
    try:
        with tempfile.TemporaryDirectory(prefix="dobi-v1-bc-recent-package-") as temporary:
    ...
    payload = {
        "schema": "ptcg.dobi-v1.bc-recent-v1.package.v1",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "internal_name": "dobi-v1-bc-recent-v1",
        ...
        "candidate": {
            ...
            "runtime_change": "replace only frozen Dobi-v1 ST_MAIN weights with the epoch-2 recent-data BC candidate",
        },
        "evidence": {
            "mirror_result": {
                "path": str(MIRROR_RESULT.resolve()),
                "sha256": sha256(MIRROR_RESULT),
                "score": mirror["decision"]["score"],
                "ci95": mirror["decision"]["wilson_ci95"],
                "noninferior": True,
                "positive_evidence": mirror["decision"]["positive_evidence"],
            },
            "field_gate_status": "not run; user requested a direct ladder experiment after mirror preservation",
        },
        "authorization": {
            "upload_authorized": False,
            "reason": "exact Kaggle submission tag must be supplied by the user",
        },
